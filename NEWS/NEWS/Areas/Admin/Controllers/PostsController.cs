...
namespace NEWS.Areas.Admin.Controllers
{
    ...
    public class PostsController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "Id,Photo,Title,Content,Dateline")] Post post, HttpPostedFileBase Photo)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(int? id,[Bind(Include = "Id,Photo,Title,Content,Dateline")] Post post, HttpPostedFileBase Photo)
        {
            if (ModelState.IsValid)
            {
                Post active = db.Posts.Find(id);
                if (active != null)
                {
                    string fileName = null;
                    if (Photo != null)
                    {
                        if (Photo.ContentLength > 0 && Photo.ContentLength <= 3 * 1024 * 1024)
                        {
                            ...
                            )
                            {
                                ...
                                active.Content = post.Content;
                                active.Dateline = post.Dateline;
                                db.SaveChanges();
                                return RedirectToAction("Index");
                            }
                            else
